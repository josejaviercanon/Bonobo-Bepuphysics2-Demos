...
/**
 * Port of the upstream `ContactEventsDemo`: the full `IContactEventHandler` layer turns
 * narrow phase manifold callbacks into contact add/remove/touching/pair events. A box and a
 * capsule drop onto the floor and wall; every newly added contact spawns a green particle.
 */
export async function createContactEventsScene(engine: Engine, canvas: HTMLCanvasElement): Promise<SceneHandle> {
    ...
    const staticSet = createShapeSet(scene, 'demo-contact-statics', 'box', new Color3(0.5, 0.55, 0.65), 2, 2);
    const bodySet = createShapeSet(scene, 'demo-contact-bodies', 'box', new Color3(0.85, 0.65, 0.35), 2, 2);
    const capsuleSet = createShapeSet(scene, 'demo-contact-capsule', 'capsule', new Color3(0.4, 0.8, 0.9), 1, 1, {
        capsuleHeight: 1.2,
        capsuleRadius: 0.25,
    });
    const particleSet = createShapeSet(scene, 'demo-contact-particles', 'sphere', new Color3(0.3, 0.95, 0.45), MAX_PARTICLES, 64, {
        sphereSegments: 8,
    });
    ...
    if (!stream) {
        console.error('[bepu-demos] sceneContactEvents: no signal stream (host bridge missing)');
    } else {
    ...
    return {
        scene,
        cleanup: () => {
            window.__contactEvents = undefined;
            stream?.close();
        },
    };
}
