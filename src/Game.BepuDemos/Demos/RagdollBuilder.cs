...
/// <summary>
///     Ragdoll construction helpers ported from the upstream BepuPhysics2 <c>RagdollDemo</c>
///     (Apache-2.0, Ross Nordby): 15 bodies per ragdoll (hips/abdomen/chest/head, two arms, two
///     legs) wired with BallSocket + SwingLimit + TwistLimit + AngularMotor bundles, with
///     subgroup-filtered self collision. Shared by <c>RagdollTubeDemo</c> and the dancer demos.
///
///     Deviations: the upstream demo renders through the demo renderer; here every body is a
///     <see cref="Transform3DState"/> record owned by the ported fixture.
/// </summary>
public static class RagdollBuilder
{
    ...
    public struct RagdollHandles
    {
        ...
        /// <summary>Appends every ragdoll body handle in a stable order (render-id slot order).</summary>
        public void CopyTo(Span<BodyHandle> target)
        {
            ...
            target[2] = Chest;
            target[3] = Head;
            target[4] = RightArm.UpperArm;
            target[5] = RightArm.LowerArm;
            target[6] = RightArm.Hand;
            target[7] = LeftArm.UpperArm;
            target[8] = LeftArm.LowerArm;
            target[9] = LeftArm.Hand;
            target[10] = RightLeg.UpperLeg;
            target[11] = RightLeg.LowerLeg;
            target[12] = RightLeg.Foot;
            target[13] = LeftLeg.UpperLeg;
            target[14] = LeftLeg.LowerLeg;
            target[15] = LeftLeg.Foot;
        }
    }

    /// <summary>Number of bodies per <see cref="AddRagdoll"/> (render-id slot stride).</summary>
    public const int BodiesPerRagdoll = 16;

    public static BodyHandle AddBody<TShape>(TShape shape, float mass, in RigidPose pose, Simulation simulation)
    ...
    public static RigidPose GetWorldPose(Vector3 localPosition, Quaternion localOrientation, RigidPose ragdollPose)
    {
        RigidPose worldPose;
    ...
    /// <summary>Builds a capsule covering a line segment plus its world pose.</summary>
    public static void GetCapsuleForLineSegment(
        Vector3 start, Vector3 end, float radius, out Capsule capsule, out Vector3 position, out Quaternion orientation)
    {
    ...
    private static AngularMotor BuildAngularMotor() =>
        new() { TargetVelocityLocalA = new Vector3(), Settings = new MotorSettings(float.MaxValue, 0.01f) };

    private static RagdollArmHandles AddArm(
        float sign, Vector3 localShoulder, RigidPose localChestPose, BodyHandle chestHandle, ref SubgroupCollisionFilter chestMask,
        int limbBaseBitIndex, int ragdollIndex, RigidPose ragdollPose, CollidableProperty<SubgroupCollisionFilter> filters,
        SpringSettings constraintSpringSettings, Simulation simulation)
    {
    ...
    private static RagdollLegHandles AddLeg(
        Vector3 localHip, RigidPose localHipsPose, BodyHandle hipsHandle, ref SubgroupCollisionFilter hipsFilter,
        int limbBaseBitIndex, int ragdollIndex, RigidPose ragdollPose, CollidableProperty<SubgroupCollisionFilter> filters,
        SpringSettings constraintSpringSettings, Simulation simulation)
    {
    ...
    /// <summary>
    ///     Builds one 15-body ragdoll at <paramref name="position"/> with subgroup-filtered self
    ///     collision; <paramref name="ragdollIndex"/> must be unique among colliding ragdolls
    ///     (it is the <see cref="SubgroupCollisionFilter.GroupId"/>).
    /// </summary>
    public static RagdollHandles AddRagdoll(
        Vector3 position, Quaternion orientation, int ragdollIndex,
        CollidableProperty<SubgroupCollisionFilter> collisionFilters, Simulation simulation)
    {
        ...
        hipsFilter = new SubgroupCollisionFilter(ragdollIndex, 0);
        abdomenFilter = new SubgroupCollisionFilter(ragdollIndex, 1);
        chestFilter = new SubgroupCollisionFilter(ragdollIndex, 2);
        headFilter = new SubgroupCollisionFilter(ragdollIndex, 3);
