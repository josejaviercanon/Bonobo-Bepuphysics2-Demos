# 1.0.0 — Bonobo.Bepuphysics2 (fork)

First packaged release of the Bonobo Engine fork of bepuphysics v2.

## Changes from upstream 2.5.0-beta.24

1. Root namespaces renamed: `BepuPhysics` → `Bonobo.Bepuphysics2`, `BepuUtilities` → `Bonobo.BepuUtilities`.
2. Assemblies renamed to `Bonobo.Bepuphysics2` / `Bonobo.BepuUtilities`; a single NuGet package `Bonobo.Bepuphysics2` bundles both.
3. Target framework moved to .NET 10 with `LangVersion` 14 and `IsAotCompatible=true`.
4. Strong-name signing removed; assemblies are unsigned.
5. `Microsoft.SourceLink.GitHub` dependency removed (zero external NuGet dependencies).
6. Project layout moved under `src/`; demos, benchmarks, and the legacy `BepuUtilitiesTests` project removed.
7. Added `src/Bonobo.Bepu.AotProbe`: native AOT publish, zero-allocation steady-state assertion, and null-dispatcher determinism check.
8. No source logic changes. Public API surface is unchanged apart from namespaces.
